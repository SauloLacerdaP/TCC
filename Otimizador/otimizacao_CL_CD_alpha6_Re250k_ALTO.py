...
Objetivo:
    ...
    com polar completa no XFOIL. Para cada geometria, o surrogate avalia exclusivamente alpha = 6 graus e Re = 250.000, usando CL/CD como função objetivo.
...
OUTPUT_DIR = ROOT / "Output_dados" / "otimizacao_CL_CD"
...
#    regiões legítimas próximas de perfis de alto CL/CD que ficaram fora do
...
#
# 8) CD e CM:
#    permanecem SEM restrição nesta otimização de CL/CD. O objetivo é:
#    maximizar CL/CD em alpha=6 graus e Re=250000. CD e CM são diagnósticos.
#
# 9) XFOIL:
...
INIT_HIGH_EFF_FRACTION = 0.70
HIGH_EFF_TOP_N_PROFILES = 20
HIGH_EFF_LOCAL_STD = 0.12
HIGH_EFF_WIDE_STD = 0.35
HIGH_EFF_WIDE_FRACTION = 0.35
...
# Na otimização de CL/CD, CD e CM permanecem como diagnóstico,
...
def gerar_populacao_inicial_factivel(
    ...
    População inicial V2:
      ...
      - se a parcela global tiver baixa aceitação, completa rapidamente
        com perturbações em regiões de alto CL/CD.
    ...
    target_high = int(round(target_size * INIT_HIGH_EFF_FRACTION))
    ...
    # --------------------------------------------------------------
    # Centros promissores: maior CL/CD na condição oficial.
    # --------------------------------------------------------------
    ref = df_ref.copy()
    mask = (
    ...
    if ref.empty:
        raise RuntimeError(
            ...
            "para selecionar centros de alto CL/CD."
        )

    # Centros promissores segundo a MESMA função objetivo: CL/CD.
    # Mantém exatamente a condição oficial Re=250k e alpha=6°.
    ref["CL_CD"] = np.where(
        pd.to_numeric(ref["CD"], errors="coerce") > 0.0,
        pd.to_numeric(ref["CL"], errors="coerce")
        / pd.to_numeric(ref["CD"], errors="coerce"),
        np.nan,
    )
    ref = ref.replace([np.inf, -np.inf], np.nan).dropna(subset=["CL_CD"])

    if "perfil" in ref.columns:
        ref = (
            ref.sort_values("CL_CD", ascending=False)
            .drop_duplicates("perfil", keep="first")
        )
    else:
        ref = ref.sort_values("CL_CD", ascending=False)

    high = ref.head(HIGH_EFF_TOP_N_PROFILES).copy()
    ...
    effs = high["CL_CD"].to_numpy(dtype=float)

    print("\nCentros de alto CL/CD/CD:")
    for i, (name, eff) in enumerate(zip(names, effs), 1):
        print(f"  {i:02d}. {name:<16s} CL/CD_base={eff:.6f}")

    print("\nGERANDO POPULAÇÃO INICIAL V2")
    print(f"Alvo total: {target_size}")
    print(f"Alvo alto-CL/CD: {target_high}")
    ...
    # --------------------------------------------------------------
    # 2) Completa os 70% perto de regiões de alto CL/CD.
    ...
    while len(population) < target_high and high_attempts < high_attempt_limit:
        ...
        sigma = (
            HIGH_EFF_WIDE_STD
            if rng.random() < HIGH_EFF_WIDE_FRACTION
            else HIGH_EFF_LOCAL_STD
        )
    ...
    print(
        f"Etapa alto-CL/CD concluída: {high_count}/{target_high} aceitos "
    ...
    while len(population) < target_size and fallback_attempts < fallback_attempt_limit:
        ...
        sigma = rng.uniform(HIGH_EFF_LOCAL_STD, HIGH_EFF_WIDE_STD)
        z = z_centers[idx] + rng.normal(
            0.0, sigma, size=z_centers.shape[1]
        )
        cst = dominio.scaler.inverse_transform(z.reshape(1, -1))[0]
    ...
    print(f"  Fallback alto-CL/CD: {len(population) - high_count - global_count}")
    print(
        f"  Tentativas: alto-CL/CD={high_attempts}, "
...
class ProblemaOtimizacao:
    ...
    Para cada geometria válida:
      ...
      4) usa -(CL/CD) como função objetivo;
    ...
    def objective(self, vector):
        ...
        # equivalente a maximizar CL/CD mantendo distância do limite.
...
def ler_polar_xfoil(path: Path):
    ...
    """Lê toda a polar XFOIL e retorna DataFrame + ponto de CL/CD em alpha=6°."""
...
def main():
    ...
    print(
        f"Inicialização: ~{100*INIT_HIGH_EFF_FRACTION:.0f}% Top-"
        f"{HIGH_EFF_TOP_N_PROFILES} alto-CL/CD + "
    ...
    resumo_de = pd.DataFrame([{
        ...
        "CL_CD_pred_obj": float(best_de["CL_CD_pred"]),
    ...
    print(f"CL/CD ML: {best_de['CL_CD_pred']:.8f}")
    ...
    print("ÓTIMO FINAL DE CL/CD CONFIRMADO PELO XFOIL EM ALPHA=6°")
    ...
    print(
        ...
        f"Surrogate: CL/CD={float(best_xfoil_row['CL_CD_pred_opt']):.8f} "
    ...
    print(
        ...
        f"XFOIL: CL/CD={float(best_xfoil_row['CL_CD_xfoil_opt']):.8f} "
    ...
    print(
        ...
        "CL/CD em alpha=6° no XFOIL entre os candidatos Top-N do surrogate."
