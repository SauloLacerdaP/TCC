...
Objetivo:
    ...
    com polar completa no XFOIL. Para cada geometria, o surrogate avalia exclusivamente alpha = 6 graus e Re = 250.000, usando CD como função objetivo.
...
OUTPUT_DIR = ROOT / "Output_dados" / "otimizacao_CD"
...
# porém o limite duro é ampliado nesta versão exploratória.
# O s1223 estava em ~1.70 x o P80; 2.25 x torna aquela região alcançável
# sem remover totalmente a noção de suporte geométrico.
...
# Regularidade geométrica baseada no diagnóstico da base.
# A curvatura é analisada apenas no trecho interno da corda para evitar
# singularidades/fortes gradientes naturais próximos ao BA e BF.
CURVATURE_X_MIN = 0.02
CURVATURE_X_MAX = 0.98
CURVATURE_N_POINTS = 2001

# Limites de inversões RELEVANTES de curvatura.
# O limiar para considerar uma inversão é 2% do P95 de |kappa| da própria
# superfície, reduzindo a contagem de ruído numérico próximo de kappa=0.
MAX_CURVATURE_INVERSIONS_UPPER = 2
MAX_CURVATURE_INVERSIONS_LOWER = 3
CURVATURE_SIGN_THRESHOLD_FRACTION = 0.02

# ======================================================================
# RESTRIÇÕES GEOMÉTRICAS — VERSÃO FINAL CD
# ======================================================================
# Objetivo: minimizar CD em Re=250000 e alpha=6 graus.
#
# Mesmas restrições geométricas usadas na otimização final de CL:
# - limites CST expandidos em 75%;
# - fechamento BA/BF conforme parametrização CST adotada;
# - não cruzamento entre extradorso e intradorso;
# - espessura máxima dentro da faixa observada na base;
# - distância multivariada ao treino limitada a P80 * 1.80;
# - |camber|max <= 9% c (máximo empírico da base + tolerância).
#
# É imposta uma restrição de REGULARIDADE GEOMÉTRICA por inversões relevantes
# de curvatura no trecho 0.02 <= x/c <= 0.98:
#   extradorso <= 2 inversões;
#   intradorso <= 3 inversões.
# O critério foi acrescentado para impedir oscilações CST artificiais observadas
# durante a minimização de CD. Curvaturas muito pequenas são ignoradas por um
# limiar relativo, evitando contar ruído numérico.
#
# Não são impostos envelopes locais por estação nem limites rígidos de magnitude
# de curvatura, pois o diagnóstico anterior mostrou sobreposição entre perfis
# válidos e inválidos nesses indicadores.
#
# Não há CL mínimo nem restrição de CM: esta execução representa o extremo
# independente de mínimo CD. CL e CM permanecem como diagnósticos.
#
# XFOIL valida os Top-N e o ranking oficial é pelo menor CD em alpha=6 graus.
# ======================================================================
...
INIT_LOW_CD_FRACTION = 0.70
LOW_CD_TOP_N_PROFILES = 20
LOW_CD_LOCAL_STD = 0.12
LOW_CD_WIDE_STD = 0.35
LOW_CD_WIDE_FRACTION = 0.35
...
def contar_inversoes_relevantes_curvatura(
    x: np.ndarray,
    y: np.ndarray,
):
    """
    Conta inversões relevantes do sinal da curvatura em 0.02 <= x/c <= 0.98.

    Curvaturas com magnitude inferior a uma fração do P95 de |kappa|
    são ignoradas antes da contagem, reduzindo falsas inversões causadas
    por ruído numérico próximo de kappa = 0.
    """
...
def diagnosticar_regularidade_curvatura(
    au: np.ndarray,
    al: np.ndarray,
):
    """Calcula métricas de regularidade por curvatura para as duas superfícies."""
...
def gerar_populacao_inicial_factivel(
    ...
    População inicial V2:
      ...
      - ~70% perto dos Top-N perfis de menor CD;
      ...
      - se a parcela global tiver baixa aceitação, completa rapidamente
        com perturbações em regiões de baixo CD.

    A validação geométrica continua ativa e o limite de distância continua
    sendo o limite AMPLIADO definido no DominioGeometrico.
    """
    rng = np.random.default_rng(seed)
    n_dim = 15 if optimize_alpha else 14
    target_size = max(5, POP_SIZE * n_dim)
    target_high = int(round(target_size * INIT_LOW_CD_FRACTION))
    ...
    # --------------------------------------------------------------
    # Centros promissores: menor CD na condição oficial.
    # --------------------------------------------------------------
    ref = df_ref.copy()
    mask = (
    ...
    if ref.empty:
        raise RuntimeError(
            ...
            "para selecionar centros de baixo CD."
        )

    if "perfil" in ref.columns:
        ref = (
            ref.sort_values("CD", ascending=True)
            .drop_duplicates("perfil", keep="first")
        )
    else:
        ref = ref.sort_values("CD", ascending=True)

    high = ref[ref["CD"] > 0.0].head(LOW_CD_TOP_N_PROFILES).copy()
    ...
    cls = high["CD"].to_numpy(dtype=float)

    print("\nCentros de baixo CD:")
    for i, (name, cd) in enumerate(zip(names, cls), 1):
        print(f"  {i:02d}. {name:<16s} CD_base={cd:.8f}")

    print("\nGERANDO POPULAÇÃO INICIAL V2")
    print(f"Alvo total: {target_size}")
    print(f"Alvo baixo-CD: {target_high}")
    ...
    # --------------------------------------------------------------
    # 2) Completa os 70% perto de regiões de baixo CD.
    ...
    while len(population) < target_high and high_attempts < high_attempt_limit:
        ...
        sigma = (
            LOW_CD_WIDE_STD
            if rng.random() < LOW_CD_WIDE_FRACTION
            else LOW_CD_LOCAL_STD
        )
    ...
    print(
        f"Etapa baixo-CD concluída: {high_count}/{target_high} aceitos "
    ...
    while len(population) < target_size and fallback_attempts < fallback_attempt_limit:
        ...
        sigma = rng.uniform(LOW_CD_LOCAL_STD, LOW_CD_WIDE_STD)
        z = z_centers[idx] + rng.normal(
            0.0, sigma, size=z_centers.shape[1]
        )
        cst = dominio.scaler.inverse_transform(z.reshape(1, -1))[0]
    ...
    print(f"  Baixo-CD: {high_count}")
    print(f"  Global: {global_count}")
    print(f"  Fallback baixo-CD: {len(population) - high_count - global_count}")
    print(
        f"  Tentativas: baixo-CD={high_attempts}, "
...
class ProblemaOtimizacao:
    ...
    Para cada geometria válida:
      ...
      3) seleciona o menor CD surrogate admissível;

      4) usa min(CD) como função objetivo;
    ...
    def predict_polar(self, au, al, re_value):
        ...
        best_idx = valid_polar["CD_pred"].idxmin()
    ...
    def _penalidade_geometrica(self, info):
        ...
        upper_inv = info.get("upper_curvature_inversions", np.nan)
        if (
            np.isfinite(upper_inv)
            and upper_inv > MAX_CURVATURE_INVERSIONS_UPPER
        ):
            penalty += PENALTY_SCALE * (
                upper_inv - MAX_CURVATURE_INVERSIONS_UPPER
            )

        lower_inv = info.get("lower_curvature_inversions", np.nan)
        if (
            np.isfinite(lower_inv)
            and lower_inv > MAX_CURVATURE_INVERSIONS_LOWER
        ):
            penalty += PENALTY_SCALE * (
                lower_inv - MAX_CURVATURE_INVERSIONS_LOWER
            )
    ...
    for idx, item in enumerate(candidates, start=1):
        ...
        row = {
            ...
            **{f"Al{i}": float(al[i]) for i in range(7)},
            ...
            "optimization_score": float(item.get("optimization_score", -item["CD_pred"])),
            ...
            "upper_curvature_inversions": int(
                item["geom_info"]["upper_curvature_inversions"]
            ),
            "lower_curvature_inversions": int(
                item["geom_info"]["lower_curvature_inversions"]
            ),
            "upper_kappa_abs_p99": float(
                item["geom_info"]["upper_kappa_abs_p99"]
            ),
            "lower_kappa_abs_p99": float(
                item["geom_info"]["lower_kappa_abs_p99"]
            ),
            "upper_kappa_total_variation": float(
                item["geom_info"]["upper_kappa_total_variation"]
            ),
            "lower_kappa_total_variation": float(
                item["geom_info"]["lower_kappa_total_variation"]
            ),
        ...
        if xf is not None:
            ...
            print(
                ...
                f"ML CD={row['CD_pred_opt']:.6f} @ {row['alpha_pred_opt']:.2f}° | "

                f"XFOIL CD={row['CD_xfoil_opt']:.6f} @ {row['alpha_xfoil_opt']:.2f}° | "
...
def comparar_com_menores_cd_banco(df_ref, top_n=10):
    """Ranking dos menores CDs reais em Re=250k e alpha=6°."""
    work = df_ref.copy()
    mask = (
        np.isclose(work["Re"].to_numpy(float), RE_FIXED, rtol=0.0, atol=1e-6)
        & np.isclose(work["alpha"].to_numpy(float), ALPHA_FIXED, rtol=0.0, atol=1e-8)
    )
    work = work.loc[mask].copy()
    work["CD"] = pd.to_numeric(work["CD"], errors="coerce")
    work = work[np.isfinite(work["CD"]) & (work["CD"] > 0.0)].copy()
    if work.empty:
        raise RuntimeError("Sem CD positivo em Re=250k e alpha=6°.")

    if "perfil" in work.columns:
        work = work.sort_values("CD").drop_duplicates("perfil", keep="first")
    else:
        work = work.sort_values("CD")

    cols = [c for c in ["perfil","Re","alpha","CL","CD","CM"] if c in work.columns]
    ranking = work[cols].head(top_n).copy()
    ranking.insert(0, "rank_CD_banco", np.arange(1, len(ranking)+1))
    ranking.to_csv(OUTPUT_DIR / "benchmark_menores_CD_banco_alpha6_Re250k.csv", index=False)

    print("\n" + "="*88)
    print("BENCHMARK — MENORES CDs DO BANCO EM RE=250K E ALPHA=6°")
    print("="*88)
    for _, r in ranking.iterrows():
        nome = str(r["perfil"]) if "perfil" in ranking.columns else "-"
        print(
            f"{int(r['rank_CD_banco']):02d}. {nome:<16s} "
            f"CD={float(r['CD']):.8f} | CL={float(r['CL']):.6f} | CM={float(r['CM']):.6f}"
        )
    return ranking


def comparar_otimizado_cd_com_banco(best_row, ranking_banco):
    """Compara o melhor CD físico otimizado com o melhor CD do banco."""
    db = ranking_banco.iloc[0]
    cd_db = float(db["CD"])
    cd_opt = float(best_row["CD_xfoil_opt"])
    delta = cd_opt - cd_db
    melhoria = 100.0 * (cd_db - cd_opt) / max(abs(cd_db), 1e-12)
    nome = str(db["perfil"]) if "perfil" in ranking_banco.columns else "melhor_banco"

    print("\n" + "="*88)
    print("COMPARAÇÃO FINAL — CD OTIMIZADO VS. BANCO")
    print("="*88)
    print(f"Melhor perfil do banco: {nome}")
    print(f"CD banco:              {cd_db:.8f}")
    print(f"CD otimizado (XFOIL):  {cd_opt:.8f}")
    print(f"Delta CD:              {delta:+.8f}")
    if cd_opt < cd_db:
        print(f"GANHO: CD otimizado {melhoria:.3f}% menor que o melhor CD do banco.")
    else:
        print(f"RESULTADO: CD otimizado {-melhoria:.3f}% maior que o melhor CD do banco.")

    pd.DataFrame([{
        "perfil_melhor_banco": nome,
        "CD_melhor_banco": cd_db,
        "CD_otimizado_xfoil": cd_opt,
        "delta_CD": delta,
        "melhoria_CD_pct": melhoria,
    }]).to_csv(OUTPUT_DIR / "comparacao_CD_otimizado_vs_banco.csv", index=False)
...
def main():
    ...
    ranking_cd_banco = comparar_com_menores_cd_banco(df_ref, top_n=10)
    ...
    print(
        f"Inicialização: ~{100*INIT_LOW_CD_FRACTION:.0f}% Top-"
        f"{LOW_CD_TOP_N_PROFILES} baixo-CD + "
    ...
    comparar_otimizado_cd_com_banco(best_xfoil_row, ranking_cd_banco)
    ...
    print("ÓTIMO FINAL DE CD CONFIRMADO PELO XFOIL EM ALPHA=6°")
    ...
    print(
        f"Surrogate: CD={float(best_xfoil_row['CD_pred_opt']):.8f} "
        f"em alpha={float(best_xfoil_row['alpha_pred_opt']):.4f}°"
    )

    print(
        f"XFOIL: CD={float(best_xfoil_row['CD_xfoil_opt']):.8f} "
    ...
    print(
        f"Inversões relevantes de curvatura: "
        f"extradorso={int(geom_info['upper_curvature_inversions'])}/"
        f"{MAX_CURVATURE_INVERSIONS_UPPER}, "
        f"intradorso={int(geom_info['lower_curvature_inversions'])}/"
        f"{MAX_CURVATURE_INVERSIONS_LOWER}"
    )
    ...
    result_dict = {
        ...
        **{f"Al{i}": float(al[i]) for i in range(7)},
        ...
        "upper_curvature_inversions": int(
            geom_info["upper_curvature_inversions"]
        ),
        "lower_curvature_inversions": int(
            geom_info["lower_curvature_inversions"]
        ),
        "max_curvature_inversions_upper": int(
            MAX_CURVATURE_INVERSIONS_UPPER
        ),
        "max_curvature_inversions_lower": int(
            MAX_CURVATURE_INVERSIONS_LOWER
        ),
        "upper_kappa_abs_p99": float(
            geom_info["upper_kappa_abs_p99"]
        ),
        "lower_kappa_abs_p99": float(
            geom_info["lower_kappa_abs_p99"]
        ),
        "upper_kappa_total_variation": float(
            geom_info["upper_kappa_total_variation"]
        ),
        "lower_kappa_total_variation": float(
            geom_info["lower_kappa_total_variation"]
        ),
