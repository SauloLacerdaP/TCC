...
Objetivo:
    ...
    buscar geometrias CST promissoras com XGBoost usando múltiplas execuções

    independentes do Differential Evolution e confirmar o ótimo final
...
# Multi-start: cinco execuções independentes do DE.
OPTIMIZATION_SEEDS = [10, 30, 50]

POP_SIZE = 20

# Reduzido de 500 para 300 por execução; a busca total passa a ter 3 starts.
MAX_ITER = 300
...
DISTANCE_SOFT_START = 0.70
DISTANCE_SOFT_WEIGHT = 15.0
...
CANDIDATE_DISTINCT_DISTANCE_STD = 0.30
...
class ProblemaOtimizacao:
    ...
    def _montar_dataframe_polar(self, model, au, al, re_value):
        ...
        """Monta as 13 condições de alpha (0 a 12°, passo 1°) em um único DataFrame."""
    ...
    def registrar_candidato(
        ...
        O ranking do pool usa a utilidade penalizada pela proximidade da
        fronteira CST. Assim, candidatos de CL/CD bruto muito alto, mas
        colados no limite do domínio, não dominam automaticamente o Top-N.
        O CL/CD bruto continua sendo salvo para comparação com o XFOIL.
        """
    ...
    def objective(self, vector):
        ...
        #   objective = -CL/CD + penalização
        # equivalente a maximizar:
        #   score = CL/CD - penalização
...
def main():
    ...
    print("OTIMIZAÇÃO HÍBRIDA MULTI-START CST: XGBOOST + DIFFERENTIAL EVOLUTION + XFOIL")
    ...
    print(
        f"Pool de candidatos: {CANDIDATE_POOL_SIZE}; "
        f"distância mínima Top-N padronizada: "
        f"{CANDIDATE_DISTINCT_DISTANCE_STD:.2f}"
    )
    ...
    print("\n" + "=" * 88)
    print("INICIANDO BUSCA MULTI-START — DIFFERENTIAL EVOLUTION")
    print("=" * 88)
    print(f"Seeds: {OPTIMIZATION_SEEDS}")
    print(f"Iterações máximas por execução: {MAX_ITER}")

    resultados_de = []
    resumo_runs = []

    calls_before_all = problema.n_calls
    valid_before_all = problema.n_valid

    for run_idx, seed in enumerate(OPTIMIZATION_SEEDS, start=1):
        problema.current_seed = int(seed)

        print("\n" + "-" * 88)
        print(
            f"MULTI-START {run_idx}/{len(OPTIMIZATION_SEEDS)} | "
            f"seed={seed}"
        )
        print("-" * 88)
        ...
        result_run = differential_evolution(
            problema.objective,
            bounds=bounds,
            strategy="best1bin",
            maxiter=MAX_ITER,
        ...
        au_run, al_run, re_value_run = problema.decode(result_run.x)
        valid_run, info_run = validar_geometria(
            au_run, al_run, dominio, t_min, t_max
        )

        polar_run = None
        best_run = None
        if valid_run:
            polar_run, best_run = problema.predict_polar(
                au_run, al_run, re_value_run
            )

        if valid_run and best_run is not None:
            dist_pen = problema._penalidade_distancia_suave(
                float(info_run["distance_to_train"])
            )
            ...
            score_run = float(best_run["CL_CD_pred"]) - float(dist_pen)
            ml_run = float(best_run["CL_CD_pred"])
            alpha_run = float(best_run["alpha"])
        else:
            dist_pen = np.nan
            score_run = -np.inf
            ml_run = np.nan
        ...
        calls_run = problema.n_calls - calls_before
        valid_calls_run = problema.n_valid - valid_before

        resumo_runs.append({
            "run": run_idx,
            "seed": int(seed),
            "scipy_success": bool(result_run.success),
            "message": str(result_run.message),
            "geometry_valid": bool(valid_run),
            "distance_to_train": float(info_run.get("distance_to_train", np.nan)),
            "distance_limit": float(dominio.distance_limit),
            "distance_soft_penalty": float(dist_pen) if np.isfinite(dist_pen) else np.nan,
            "alpha_pred_opt": alpha_run,
            "CL_CD_pred_opt": ml_run,
            "optimization_score": score_run if np.isfinite(score_run) else np.nan,
            "objective_calls": int(calls_run),
            "valid_calls": int(valid_calls_run),
        })

        resultados_de.append({
            "seed": int(seed),
            "result": result_run,
            "valid": bool(valid_run),
            "geom_info": info_run,
            "polar": polar_run,
            "best": best_run,
            "score": score_run,
        ...
        print(f"Sucesso scipy: {result_run.success}")
        print(f"Geometria válida: {valid_run}")
        print(
            f"Distância ao treino: "
            f"{info_run.get('distance_to_train', np.nan):.6f} / "
            f"{dominio.distance_limit:.6f}"
        )
        if best_run is not None:
            print(
                f"CL/CD ML={best_run['CL_CD_pred']:.6f} @ "
                f"alpha={best_run['alpha']:.2f}° | "
                f"penalização={dist_pen:.6f} | score={score_run:.6f}"
            )
        print(f"Avaliações desta execução: {calls_run}")
        print(f"Avaliações válidas desta execução: {valid_calls_run}")
        print(f"Pool global acumulado: {len(problema.best_candidates)} candidatos")

    # Escolhe a melhor execução segundo a MESMA função objetivo penalizada.
    runs_validos = [
        r for r in resultados_de
        if r["valid"] and r["best"] is not None and np.isfinite(r["score"])
    ]
    if not runs_validos:
        raise RuntimeError(
            "Nenhuma das execuções multi-start retornou solução válida."
        )

    melhor_run = max(runs_validos, key=lambda r: r["score"])
    result = melhor_run["result"]
    best_seed = melhor_run["seed"]
    au_de, al_de, re_value = problema.decode(result.x)
    valid_de, geom_info_de = validar_geometria(
        au_de, al_de, dominio, t_min, t_max
    )
    ...
    pd.DataFrame(resumo_runs).to_csv(
        OUTPUT_DIR / "resumo_multistart_de.csv",
        index=False,
    )

    print("\n" + "=" * 88)
    print("DIAGNÓSTICO DA MELHOR EXECUÇÃO MULTI-START SEGUNDO O SURROGATE")
    print("=" * 88)
    print(f"Seed vencedora: {best_seed}")
    ...
    print(f"Avaliações totais multi-start: {problema.n_calls - calls_before_all}")
    print(f"Avaliações válidas multi-start: {problema.n_valid - valid_before_all}")
    print(f"Candidatos no pool global: {len(problema.best_candidates)}")
    ...
    result_dict = {
        ...
        **{f"Al{i}": float(al[i]) for i in range(7)},
        ...
        "optimization_seeds": [int(s) for s in OPTIMIZATION_SEEDS],
        ...
        "multistart_runs": int(len(OPTIMIZATION_SEEDS)),
    ...
    print(" - resumo_multistart_de.csv")
